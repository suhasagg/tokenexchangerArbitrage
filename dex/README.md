...
This challenge will help you build/understand a simple decentralized exchange, with one token-pair (ERC20 AXIELOONS ($AXIE) and ETH). This repo is an updated version of the [original tutorial](https://medium.com/@austin_48503/%EF%B8%8F-minimum-viable-exchange-d84f30bd0c90) and challenge repos before it. Please read the intro for a background on what we are building first!
...
> ❗️ NOTE: The current front-end may need to be refreshed as you carry out transactions to update your balance of axies in the UI, etc.
...
Navigate to the Debug Contracts tab and you should see two smart contracts displayed called `DEX` and `Axies`.
...
`Axies.sol` is just an example ERC20 contract that mints 1000 $AXIE to whatever address deploys it.
...
We want to create an automatic market where our contract will hold reserves of both ETH and 🎈 Axies. These reserves will provide liquidity that allows anyone to swap between the assets.
...
Calling `init()` will load our contract up with both ETH and 🎈 Axies.
...
Run `yarn deploy`. The front end should show you that you have axie tokens. We can’t just call `init()` yet because the DEX contract isn’t allowed to transfer tokens from our account. We need to `approve()` the DEX contract with the Axies UI.

🤓 Copy and paste the DEX address and then set the amount to 5000000000000000000 (5 _ 10¹⁸). You can confirm this worked using the `allowance()` function. Now we are ready to call `init()` on the DEX. We will tell it to take (5 _ 10¹⁸) of our tokens and we will also send 0.01 ETH with the transaction. You can see the DEX contract's value update and you can check the DEX token balance using the balanceOf function on the Axies UI.

This works pretty well, but it will be a lot easier if we just call the `init()` function as we deploy the contract. In the `00_deploy_your_contract.js` script try uncommenting the init section so our DEX will start with 5 ETH and 5 Axies of liquidity:
...
- [ ] 🎈 Under the debug tab, does your DEX show 5 ETH and 5 Axies of liquidity?
...
🤔 OH! A market based on a curve like this will always have liquidity, but as the ratio becomes more and more unbalanced, you will get less and less of the less-liquid asset from the same trade amount. Again, if the smart contract has too much ETH and not enough $AXIE tokens, the price to swap $AXIE tokens to ETH should be more desirable.

When we call `init()` we passed in ETH and $AXIE tokens at a ratio of 1:1. As the reserves of one asset changes, the other asset must also change inversely in order to maintain the constant product formula (invariant k).
...
```
    /**
     * @notice sends Ether to DEX in exchange for $AXIE
     */
    function ethToToken() public payable returns (uint256 tokenOutput) {
        ...
        emit EthToTokenSwap(msg.sender, "Eth to Axies", msg.value, tokenOutput);
        return tokenOutput;
    }

    /**
     * @notice sends $AXIE tokens to DEX in exchange for Ether
     */
    function tokenToEth(uint256 tokenInput) public returns (uint256 ethOutput) {
        ...
        emit TokenToEthSwap(msg.sender, "Axies to ETH", ethOutput, tokenInput);
        return ethOutput;
    }
```
...
> 💡 _Hints:_
> The `deposit()` function receives ETH and also transfers $AXIE tokens from the caller to the contract at the right ratio. The contract also tracks the amount of liquidity (how many liquidity provider tokens (LPTs) minted) the depositing address owns vs the totalLiquidity.

> 💡💡 _More Hints:_ The `withdraw()` function lets a user take both ETH and $AXIE tokens out at the correct ratio. The actual amount of ETH and tokens a liquidity provider withdraws could be higher than what they deposited because of the 0.3% fees collected from each trade. It also could be lower depending on the price fluctuations of $AXIE to ETH and vice versa (from token swaps taking place using your AMM!). The 0.3% fee incentivizes third parties to provide liquidity, but they must be cautious of [Impermanent Loss (IL)](https://www.youtube.com/watch?v=8XJ1MSTEuU0&t=2s&ab_channel=Finematics).
...
Remember that you will need to call `approve()` from the `Axies.sol` contract approving the DEX to handle a specific number of your $AXIE tokens. To keep things simple, you can just do that when interacting with the UI or debug tab with your contract.
...
- [ ] 🧐 What happens if you `deposit()` at the beginning of the deployed contract, then another user starts swapping out for most of the axies, and then you try to withdraw your position as a liquidity provider? Answer: you should get the amount of liquidity proportional to the ratio of assets within the isolated liquidity pool. It will not be 1:1.
...
- [ ] `approve()` event emission: can you implement this into the event tabs so that it is clear when `approve()` from the `Axies.sol` contract has been executed?
