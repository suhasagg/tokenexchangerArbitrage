...
describe("🚩 Challenge 3: ⚖️ 🪙 Simple DEX", function () {
  ...
  before(async function () {
    ...
    await deployments.fixture(['Axies', 'DEX']);

    dexContract = await ethers.getContract('DEX', deployer);
    axiesContract = await ethers.getContract('Axies', deployer);
  });
  ...
  describe("DEX: Standard Path", function () {
    ...
    describe("init()", function () {
      describe("ethToToken()", function () {
        it("Should send 1 Ether to DEX in exchange for _ $AXIE", async function () {
      ...
      describe("tokenToEth", async () => {
        it("Should send 1 $AXIE to DEX in exchange for _ $ETH", async function () {
          const axies_bal_start = await axiesContract.balanceOf(dexContract.address);
          ...
          expect(await axiesContract.balanceOf(dexContract.address))
            .to.equal(axies_bal_start.add(ethers.utils.parseEther("1")));
        });
      ...
      describe("deposit", async () => {
        it("Should deposit 1 ETH and 1 $AXIE when pool at 1:1 ratio", async function () {
      ...
      // pool should have 5:5 ETH:$AXIE ratio
      describe("withdraw", async () => {
        it("Should withdraw 1 ETH and 1 $AXIE when pool at 1:1 ratio", async function () {
          let tx1 = await dexContract
